...
class Encoder(nn.Module):
    ...
    def forward(self,input):
        assert input.shape[1]==self.in_c #Assume input=(b,1,256,256)
        
        layer_feats = {}
        ef1 = self.e1(input) #ef1 = (b,64,128,128)
        layer_feats['ef1']=ef1 
        ef2 = self.e2(ef1) #ef2 = (b,128,64,64)
        layer_feats['ef2']=ef2
        ef3 = self.e3(ef2) #ef3 = (b,256,32,32)
        layer_feats['ef3']=ef3
        ef4 = self.e4(ef3) #ef4 = (b,512,16,16)
        layer_feats['ef4']=ef4
        ef5 = self.e5(ef4) #ef5 = (b,512,8,8) 
        layer_feats['ef5']=ef5
        ef6 = self.e6(ef5) #ef6 = (b,512,4,4)
        layer_feats['ef6']=ef6
...
class Decoder(nn.Module):
    ...
    def forward(self,ef7,layer_feats):
        ...
        df5 = F.dropout(self.l6(torch.cat((df6,layer_feats['ef6']),dim=1))) # df5=(b,512,8,8)
        df4 = F.dropout(self.l5(torch.cat((df5,layer_feats['ef5']),dim=1))) # df4=(b,512,16,16)
        df3 = self.l4(torch.cat((df4,layer_feats['ef4']),dim=1)) #df3=(b,256,32,32)
        df2 = self.l3(torch.cat((df3,layer_feats['ef3']),dim=1)) #df2=(b,128,64,64)
        df1 = self.l2(torch.cat((df2,layer_feats['ef2']),dim=1)) #df1=(b,64,128,128)
        output = self.tanh(self.l1(torch.cat((df1,layer_feats['ef1']),dim=1))) #output=(b,5,256,256)
